...
async function fetchLinkedInToken(code: string, redirectUri: string) {
  ...
  // Log all parameters except client_secret
  console.log('[LinkedIn Token Exchange] Params:', {
  ...
  if (!res.ok) {
    ...
    console.error('[LinkedIn Token Error]', res.status, errorBody);
    throw new Error(`Failed to fetch LinkedIn access token: ${res.status} ${errorBody}`);
  }
  return res.json();
}
...
export async function GET(req: NextRequest) {
  ...
  if (!code) {
    console.error('[LinkedIn Callback] Missing code param. URL:', req.url, 'Query:', Object.fromEntries(searchParams.entries()));
    return new NextResponse(
      '<html><body><h2>LinkedIn connection failed</h2><p>Missing code parameter. Please try connecting again from your profile page, or ensure you are not visiting this URL directly.</p></body></html>',
  ...
  try {
    ...
    const { error: dbError } = await supabase.from('user_linkedin_data').upsert({
      user_id,
      provider: 'linkedin',
      profile,
      email,
